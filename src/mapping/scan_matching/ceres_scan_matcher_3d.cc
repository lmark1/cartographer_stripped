...
#include "cartographer_stripped/mapping/rotation_parameterization.h"
#include "cartographer_stripped/mapping/scan_matching/occupied_space_cost_function_3d.h"
#include "cartographer_stripped/mapping/scan_matching/rotation_delta_cost_functor_3d.h"
#include "cartographer_stripped/mapping/scan_matching/translation_delta_cost_functor_3d.h"
#include "cartographer_stripped/optimization/ceres_pose.h"
  ...
  common::LuaParameterDictionary* const parameter_dictionary)
{
  ...
  for (int i = 0;; ++i) {
    ...
    if (!parameter_dictionary->HasKey(lua_identifier)) { break; }
...
CeresScanMatcher3D::CeresScanMatcher3D(const proto::CeresScanMatcherOptions3D& options)
  : options_(options), ceres_solver_options_(
  ...
  ceres::Solver::Summary* const                       summary)
{
  ...
  optimization::CeresPose ceres_pose(
    ...
    nullptr /* translation_parameterization */,
    options_.only_optimize_yaw()
      ? std::unique_ptr<ceres::LocalParameterization>(
          std::make_unique<
            ceres::AutoDiffLocalParameterization<YawOnlyQuaternionPlus, 4, 1>>())
      : std::unique_ptr<ceres::LocalParameterization>(
  ...
  for (size_t i = 0; i != point_clouds_and_hybrid_grids.size(); ++i) {
    ...
    const sensor::PointCloud& point_cloud = *point_clouds_and_hybrid_grids[i].first;
    ...
    problem.AddResidualBlock(OccupiedSpaceCostFunction3D::CreateAutoDiffCostFunction(
                             ...
                             ceres_pose.translation(),
                             ceres_pose.rotation());
  }
  CHECK_GT(options_.translation_weight(), 0.);
  problem.AddResidualBlock(
    TranslationDeltaCostFunctor3D::CreateAutoDiffCostFunction(
      options_.translation_weight(), options_.translation_weight_z(), target_translation),
    nullptr /* loss function */,
    ceres_pose.translation());
