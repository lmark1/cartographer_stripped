...
std::unique_ptr<cartographer_stripped::transform::Rigid3d> TfBridge::LookupToTracking(
    const cartographer_stripped::common::Time time,
    const std::string& frame_id) const {
  ros::Duration timeout(lookup_transform_timeout_sec_);
  std::unique_ptr<cartographer_stripped::transform::Rigid3d> frame_id_to_tracking;
  try {
    ...
    return std::make_unique<cartographer_stripped::transform::Rigid3d>(
